...
such as deterrence flash lights and speakers. Includes connection auto-recovery,
command dispatching, response listening, and heartbeat telemetry.
"""
...
class ArduinoController:
    ...
    def __init__(
        ...
        """
        Initializes the ArduinoController and establishes initial serial connection.

        Args:
            port (Optional[str]): Serial COM port (e.g., 'COM3' or '/dev/ttyUSB0').
            baud_rate (Optional[int]): Baud rate for serial communication (default: 9600).
        ...
        # Attempt automatic connection upon instantiation
        self.connect()
    ...
    def connect(self) -> bool:
        ...
        if self.serial_conn and self.serial_conn.is_open:
            logger.debug(f"Serial port {self.port} is already open.")
            return True
        ...
        try:
            ...
            # Give Arduino 2 seconds to reset after DTR toggle
            time.sleep(2.0)
            
            # Read startup handshake response if available
    ...
    def _ensure_connection(self) -> bool:
        ...
        logger.warning("Arduino connection disconnected or lost. Attempting auto-reconnect...")
        return self.connect()
    ...
    def send_command(self, command: str) -> bool:
        ...
        Args:
            command (str): Plain text command (e.g., 'SPEAKER_ON', 'FLASH_OFF').

        Returns:
            bool: True if command was written successfully, False otherwise.
        """
        if not self._ensure_connection():
            logger.error(f"Cannot send command '{command}': Arduino is disconnected.")
            return False
    ...
    def read_response(self) -> Optional[str]:
        ...
        Returns:
            Optional[str]: Stripped response line (e.g., 'OK', 'READY', 'ERROR'), or None.
        """
        ...
        try:
            if self.serial_conn.in_waiting > 0:
                ...
                if line:
                    logger.debug(f"Received response from Arduino: '{line}'")
                    return line
            return None
        except SerialException as e:
            logger.error(f"SerialException reading response from Arduino: {e}")
            self.disconnect()
            return None
        except Exception as e:
    ...
    # --------------------------------------------------------------------------
    # Hardware Specific Command Methods
    # --------------------------------------------------------------------------
    def speaker_on(self) -> Optional[str]:
        ...
        Returns:
            Optional[str]: Arduino response status if received.
        """
    ...
    def heartbeat(self) -> Optional[str]:
        """
        Sends a HEARTBEAT signal to check Arduino operational readiness.

        Returns:
            Optional[str]: Response status ('OK', 'READY') or None if unreachable.
        """
...
# Global singleton instance for module-level usage
